...
| 20 | `destek-v1.html` | **TAMAM** | Destek merkezi hub'ı — açık talepler + hazır cevaplar + "çözüm bulamadın mı" köprüsü. 🔵 **Dalga 3'ün konusu** (kanon uyarlaması). | `:164`, `:274`, `:336` |
| 21 | `destek-talepleri-v1.html` | **EKSİK** | Durum sayısı **3** (`acik` 3 · `yanitlandi` 2 · `kapandi` 3), kanon **4** istiyor (`açık · yanıt bekleyen · çözülen · kapatılan`). **Sayfalandırma yok.** 🔵 **Dalga 3'ün konusu — bu turda yapılmaz.** | `data-durum` sayımı; sözleşme §2.2 |
| 22 | `destek-talebi-detay-v1.html` | **EKSİK** | **"Yeniden aç" yok** (grep 0). Talep kapatma **kalıcı değil** — sayfanın kendi notu söylüyor: yenilenince demo verisi geri geliyor. 🔵 **Dalga 3'ün konusu.** ⚠ `D4` (yeniden aç kanona girecek mi) sözleşme §1.7'de 🔴 AÇIK. | `:755-760`; grep "yeniden aç" → 0 |
...
| Y5 | **Çözüm merkezi** (üye + halka açık, iki ayrı adres) | Diet (`/hesabim/destek/cozum-merkezi` · `/cozum-merkezi`) | Fit'te kavram yok; en yakını `sss-v1.html` (halka açık SSS) | ⚠ Oturup oturmadığı **Dalga 3'ün** konusu — destek kanonu Diet'te ve henüz Fit'e kopyalanmadı |
| Y6 | **Talep sayfalandırması** | Gastro (`paginate(10)->withQueryString()`) | Fit'te yok (Dalga 1'de arandı, bulunamadı) | **Oturur**, ama 🔵 **Dalga 3** |
...
|---|---|
| Destek kanonu uyarlaması (4 durum · yeniden aç · sayfalama) | 🔵 **Dalga 3.** Kanon Diet'tedir, henüz Fit'e kopyalanmadı |
