...
| `antrenor-kazanc-tutarlari` | **Antrenörün hakediş tutarları** (brüt · net) | `antrenor-panelim-v1.html` → "Kazançlarım" bölümü: "Bu dönem" özetinin iki hücresi + hakediş defterinin brüt/net sütunları | `<span class="ap-yt" data-yer-tutucu="antrenor-kazanc-tutarlari">` — tutar yerine "Tahsilat kaydı henüz yok" / "Bekliyor"; `aria-label` durumu anlatıyor. 🔴 **Sayı UYDURULMADI:** brüt tutar paket ücretine, net tutar komisyon oranına bağlıdır; ikisi de henüz yok (bkz. `antrenor-aylik-paket-ucreti` ve `antrenor-komisyon-parametreleri`). Örnek tutar yazmak, oranın kararlaşmış olduğu izlenimini verirdi | Paket ücreti ve komisyon oranı bağlandıktan sonra hücrelerin metnini gerçek tutarla değiştir; `.ap-yt` sarmalayıcısını kaldırıp `.fd-tablo`nun kendi `td.sag` biçimine dön; `data-yer-tutucu` niteliğini, `aria-label`ı ve üstündeki YER TUTUCU yorumunu sil; `tests/antrenor-panelim.mjs` §4'teki yer-tutucu beklentisini güncelle |
| `antrenor-komisyon-parametreleri` | **K13'ün para parametreleri** — komisyon oranı · ödeme alt sınırı · fatura eşiği | `antrenor-panelim-v1.html` → "Kazançlarım": "Bu dönem" özetinin komisyon hücresi · hakediş defterinin komisyon sütunu · "Ödeme kuralların" kartının kapanış satırı | `<span class="ap-yt" data-yer-tutucu="antrenor-komisyon-parametreleri">` — oran yerine "Oran panelden okunacak" / "Panelden"; `aria-label` durumu anlatıyor. 🔴 **K13 bağlayıcı:** bu altı değer *"yönetim panelinden okunur, hiçbiri koda gömülmez"* ve o panel bu turda YOK. Kuralların KENDİSİ metin olarak yazılı (dört madde), yalnız SAYILARI bekliyor | Yönetim paneli kurulup değerler tanımlanınca hücrelerin metnini panelden okunan oran/alt sınır/eşikle değiştir; değerler **koda gömülmez**, panelden okunur; `data-yer-tutucu` niteliğini, `aria-label`ı ve YER TUTUCU yorumunu sil; `tests/antrenor-panelim.mjs` §4'teki beklentiyi güncelle |
