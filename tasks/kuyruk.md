...
| K36 | R14-B/#5 · Plan bitince arşive düşsün (`plan.durum='tamamlandi'` + `plan.bitis`) | **BITTI** | `plan.durum`/`plan.bitis`/`arsivlendi` mührü (şema v4) · kabukta `planArsivle()` · arşiv satırı iki kaynağı ayırıyor (`kaynak` alanı, plan→"gün", program→"antrenman", rozet Plan/Program) · **oran tutarsızlığı yakalandı**: arşiv gün sayısından %100 derken bitiş kartı %93 diyordu — plan kaydı kendi hareket-düzeyi oranını taşıyor artık |
| K37 | R14-B/#6 · Bitiş özet kartı (ad · gün · hareket · tam/yarım/atlandı · kuruluş+bitiş tarihi · "Yeni plan kur"/"Arşive git") | **BITTI** | `.fp-card`+`.fp-head`+`.pp-ad`+`.pp-alt`+`.fp-meta`+`.pp-eylem` — yeni bileşen yok · döküm renkleri işaret düğmeleriyle aynı (tam `--fit` · yarım `#8a6410` · atlandı `--slate-2`) · 4 genişlikte çakışma 0, kart↔plan arası 22px (R13 kanonu), taşma 0 · **kapatma TUR NUMARASIna bağlandı**: bayrakla tutunca 2. tur bitişinde kart bir daha hiç açılmıyordu · aynı gün kurulup biten planda tarih iki kez yazılıyordu, tek cümleye indi |
| K38 | R14-B/#7 · `bildirimler-v1:344` sabit taslak metni veriye bağla | **BITTI** | `bildirimler[]` şeması + `bildirimEkle/Okundu/Sil` · sayfa JS'i mevcut bloktan ÖNCE basıyor ki satırlar mevcut dinleyicileri miras alsın · örnek satır gerçek kayıt gelince kalkıyor · demo cümlesi kaçının gerçek olduğunu söylüyor · okundu VE silme depoya yazılıyor, silme geri alınabilir (4→3 yenilemeye dayandı, geri al 3'e döndürdü) |
| K39 | R14-B · Bitmiş planı YENİDEN BAŞLAT (aynı plan, işaretler sıfır, yeni tur) | **BITTI** | `yenidenBasla()` — `ilerleme`+`gunDurum` sıfır, `tur`+1, `arsivlendi` mührü açılır · düğme yalnız BİTMİŞ planda görünür + onay ister · ölçüm: 1. tur arşivde kaldı, 2. tur bitince ikinci kayıt yanına yazıldı (t1 %100 · t2 %100), bitiş kartı yeni tur için tekrar açıldı |
